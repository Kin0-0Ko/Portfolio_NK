...
export default function Wizard(props) {
  const { nodes, materials } = useGLTF(`/Portfolio_NK/models/wizard-transformed.glb`)

  const modelRef = useRef();
  useFrame((state, delta, xrFrame) => {
	modelRef.current.position.y = -1.5 + Math.sin(state.clock.elapsedTime) * 0.15
	modelRef.current.position.x = Math.cos(state.clock.elapsedTime) * 0.15
  })
